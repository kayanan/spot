...
export const findReservationPaymentById = async (id: string) => {
    return await ReservationPaymentDTO.findOne({ _id: id, isDeleted: false }).populate("reservation paidBy customer");
  };

export const findAllReservationPayments = async () => {
  return await ReservationPaymentDTO.find({ isDeleted: false }).populate("reservation paidBy customer");
};

export const findReservationPaymentsByReservation = async (reservationId: string) => {
  return await ReservationPaymentDTO.find({ reservation: reservationId, isDeleted: false }).populate("paidBy customer");
};

export const findReservationPaymentsByCustomer = async (customerId: string) => {
  return await ReservationPaymentDTO.find({ customer: customerId, isDeleted: false }).populate("reservation");
};
...
export const findReservationPaymentsByParkingSlot = async (parkingSlotId: string) => {
  return await ReservationPaymentDTO.find({ parkingSlot: parkingSlotId, isDeleted: false }).populate("reservation paidBy");
};
